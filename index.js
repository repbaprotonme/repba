...
panel.gallerybar = function ()
{
    this.draw = function (context, rect, user, time)
    {
        ...
        {
            gridlst = title.split("/");
            gridlst.push(infobj.value());
        }

        var rows = gridlst.length;
        var rh = 30;
...
},
{
 	...
 	leftright: function (context, x, y, ctrl, shift, alt, type)
    {
        ...
        obj.addperc(type == "wheeleft" ?  0.01 : -0.01);
        context.refresh()
    },
},
{
...
},
{
    ...
    updown: function (context, x, y, ctrl, shift, alt, type)
    {
        ...
        {
            stretchobj.value().add(type=="wheelup"?1:-1);
            context.refresh();
        }
        else
        {
...
},
{
    ...
    {
        var obj = context.obj;
        obj.add(context.canvas.scale &&
            scale<context.canvas.scale?-1:1);
        contextobj.reset();
        context.canvas.scale = scale;
    },
    pinchstart: function (context, rect, x, y)
...
},
{
    ...
    pressup: function (context, rect, x, y)
    {
        ...
        if (canvas.shiftKey &&
            ...
            galleryobj.noui = galleryobj.noui?0:1;
        context.refresh();
    },
    press: function (context, rect, x, y)
    {
...
},
{
	...
	tap: function (context, rect, x, y, shift, ctrl)
	{
        ...
        else if (_5cnv.sliceobj.length() > 1 && context.chapterect && context.chapterect.hitest(x,y))
        {
            _5cnv.timeobj.set(0);
...
},
{
    ...
    draw: function (context, rect, user, time)
    {
        ...
        function foo()
        {
            if (!context.canvas.infolst)
            {
                context.canvas.infolst = [];
                context.canvas.infolst[0] = `${time+1} of ${galleryobj.length()}`;
                ...
                for (var n = 0; n < keys.length; ++n)
                {
                    ...
                    if (value && value.length && typeof value === 'string')
                        context.canvas.infolst.push(value);
                }
            }
            ...
            a.draw(context, rect, context.canvas.infolst, 0);
        }
...
menuobj.draw = function()
{
    ...
    infobj.data[0] = `${menuobj.centered+1} of ${galleryobj.length()}`;
//    infobj.data[0] = `${isvisiblecount} of ${context.canvas.visibles.length}`;
/*
...
galleryobj.init = function (obj)
{
    ...
    _5cnv.sliceobj.data =
    [
        {title:"All Images", func: function()
            {
                localobj.gallery = 0;
                localobj.filter = "";
                galleryobj.init();
