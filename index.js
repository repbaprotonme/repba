...
buttonobj.reset = function()
{
    ...
    if (min && max)
    {
        ...
        var s = Math.floor(buttonobj.length()/2);
        var j = localobj.button >= 0 ? localobj.button : s;
        buttonobj.set(s);
    }
    else
    {
        ...
        buttonobj.data = [];
        var j = 180;
        var b = Math.floor(height*5);
        for (var n = j; n < b; ++n)
            buttonobj.data.push(n);
        var b = 1;
        var s = Math.floor(height/b);
        var e = s;//localobj.button >= 0 ? localobj.button : s;
        var k = buttonobj.data.findIndex(function(a){return a == e});
        buttonobj.set(k);
        var width = height*a;
...
CanvasRenderingContext2D.prototype.refresh = function (skip=1)
{
    var context = this;
...
},
{
    ...
    {
        var obj = buttonobj;
        var k = scale<context.canvas.scale?-0.005:0.005;
        obj.addperc(k);
